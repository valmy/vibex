...
class TestDecisionAccuracyRegression:
    ...
    async def test_uptrend_pattern_recognition(
        ...
        try:
            ...
            logger.info(f"Uptrend pattern decision: {result.decision.action}")
            logger.info(f"Confidence: {result.decision.confidence}%")
            logger.info(f"Risk level: {result.decision.risk_level}")
    ...
    async def test_downtrend_pattern_recognition(
        ...
        try:
            ...
            # Log the results for analysis
            logger.info(f"Downtrend pattern decision: {result.decision.action}")
            logger.info(f"Confidence: {result.decision.confidence}%")
            logger.info(f"Risk level: {result.decision.risk_level}")
            logger.info(f"Current price: ${mock_context.market_data.current_price:.2f}")
            logger.info(f"24h change: {mock_context.market_data.price_change_24h:.2f}%")
    ...
    async def test_consolidation_pattern_recognition(
        ...
        try:
            ...
            mock_context.market_data.technical_indicators = indicators_result
            mock_context.market_data.current_price = market_data[-1].close
            mock_context.market_data.price_change_24h = 0.5  # Minimal change for consolidation
            mock_context.market_data.volatility = 0.01  # Low volatility
            ...
            # Log the results for analysis
            logger.info(f"Consolidation pattern decision: {result.decision.action}")
            logger.info(f"Confidence: {result.decision.confidence}%")
            logger.info(f"Risk level: {result.decision.risk_level}")
            logger.info(f"Current price: ${mock_context.market_data.current_price:.2f}")
    ...
    async def test_breakout_pattern_recognition(
        ...
        try:
            ...
            mock_context.market_data.technical_indicators = indicators_result
            mock_context.market_data.current_price = market_data[-1].close
            mock_context.market_data.volume_24h = sum(candle.volume for candle in market_data[-24:])
            mock_context.market_data.price_change_24h = 8.0  # Strong positive change for breakout
            ...
            # Log the results for analysis
            logger.info(f"Breakout pattern decision: {result.decision.action}")
            logger.info(f"Confidence: {result.decision.confidence}%")
            logger.info(f"Risk level: {result.decision.risk_level}")
            logger.info(f"Current price: ${mock_context.market_data.current_price:.2f}")
