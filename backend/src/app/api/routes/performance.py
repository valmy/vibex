...
from ...schemas.performance_metric import PerformanceMetricRead, PerformanceMetricListResponse
...
async def list_performance_metrics(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await db.execute(
            select(PerformanceMetric).offset(skip).limit(limit)
        )
...
async def get_performance_metric(metric_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        result = await db.execute(select(PerformanceMetric).where(PerformanceMetric.id == metric_id))
